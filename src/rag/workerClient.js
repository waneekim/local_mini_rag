...
export class WorkerClient {
  ...
  async extract(source) {
    ...
    return new Promise((resolve, reject) => {
      this._run([], payload, resolve, reject);
    });
  }

  async extractGlossaryRows(source) {
    const payload = JSON.stringify({
      sourceId: source.id,
      kind: source.kind,
      title: source.title,
    ...
    return new Promise((resolve, reject) => {
      this._run(["--glossary"], payload, resolve, reject);
    });
  }

  _run(args, payload, resolve, reject) {
    const child = spawn(this.pythonCommand, [this.scriptPath, ...args], {
