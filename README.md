...
**검수 워크스페이스** (Agent 우클릭 → **검수(문장 교정)**) — 위 통합 검수를 화면으로 제공합니다:
- **원문 인라인 마킹**: 문제 단어에 색 밑줄 (🟡 비권장 · 🔴 금지/미등록, 호버 시 이유)
- **✍️ 교정 추천**: 완성된 교정문 + [교정문 복사] + 접힌 "이유 보기"
- **📖 단어 검수**: 단어별 카드(🟢승인 🟡대체추천 🔴금지/미등록, 정의·권장 대체어), 미등록어는
  **"용어집에 등록하러 가기"** 버튼으로 바로 등재 흐름 연결
- **✍️ 스타일 규칙**: 규칙 위반 + 원칙 표시
