...
회사 API Key 없이 **내 PC에서 전부 도는** 구성입니다. 앱 좌측 패널 하단 ⬇️(내 PC에 설치)에도
같은 가이드가 들어 있습니다.
...
**정리 카드** — 사양서처럼 같은 주제가 여러 문서에 흩어져 있을 때, 확정된 개념마다 링크된
청크들(소스 횡단)을 LLM이 종합해 **하나의 정리 카드**를 만듭니다: 정의 · 조건 · 유발 동작 ·
표기 차이 · **⚠️ 소스 간 불일치**(문서끼리 조건/수치가 다르면 명시). 중복 기술은 병합됩니다.

- 카드는 `🧠 개념 정리 카드` 시스템 소스 아래 **검색 가능한 청크로 색인**되어, 질문 시 깔끔한
  카드가 1차 히트가 되고 근거 번호 [n]으로 원본 청크까지 드릴다운됩니다.
- 개념별 생성/재생성(개념 행의 📄 버튼) 또는 **전체 정리 카드 생성**(백그라운드 잡, 개념당 LLM 1회).
- 재생성 시 이전 카드를 교체하고, 개념 삭제 시 카드도 함께 제거됩니다.
...
좌측 패널 상단의 소스 도구(📤 파일 · 📁 폴더 · 📄 텍스트 · 🔗 URL), Agent 우클릭 메뉴, 드래그앤드롭, 슬래시 명령으로 추가할 수 있습니다.
...
### 자동 임베딩
좌측 소스 도구의 **⚡ 토글**(켜면 강조) 또는 `/autoindex on|off`. 켜두면 소스를 추가하는 즉시
...
- **자동 승인**: 좌측 소스 도구의 **✨ 토글** 또는 `/autoapprove on`을 켜면 검토 단계 없이 구조화 →
  색인까지 한 번에 진행됩니다.
...
폴더를 통째로 추가할 때 좌측 소스 도구의 **🗂 토글**(또는 `/foldertree on`)을 켜면, 파일들과 함께
...
- `POST /api/profiles/:id/concepts/:conceptId/card` (정리 카드 생성) · `POST .../concepts/cards` (전체 카드 잡)
